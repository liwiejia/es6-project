...
var makeIssue = function() {
    var date = new Date();
    var frist_issue_date = new Date();
    frist_issue_date.setHours(9);
    frist_issue_date.setMinutes(10);
    frist_issue_date.setSeconds(0);
    var end_issue_date = new Date(frist_issue_date.getTime() + 77 * 10 * 60 * 1000);

    var cur_issue, end_time, state;
    if (date.getTime() - frist_issue_date.getTime() > 0 && date.getTime() - end_issue_date.getTime() < 0) {
        //正常销售
        ...
        cur_issue = end_date.getTime();
        cur_issue = [end_date.getFullYear(), ('0' + (end_date.getMonth() + 1)).slice(-2), ('0' + end_date.getDate()).slice(-2), ('0' + h).slice(-2)].join('');
    } else {
        //今天销售截止
        frist_issue_date.setDate(frist_issue_date.getDate() + 1);
        end_time = frist_issue_date.getTime();
        cur_issue = [frist_issue_date.getFullYear(), ('0' + (frist_issue_date.getMonth() + 1)).slice(-2), ('0' + frist_issue_date.getDate()).slice(-2), '01'].join('');
    }
